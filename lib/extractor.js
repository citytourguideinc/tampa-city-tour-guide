...
const JUNK_TITLES = [
  ...
  /^all\s+\w/i, 
  /page\s+\d+/i, 
  /^(events|news|newsletter|restaurants|things to do|tours|activities)$/i
];
...
function extractTitle(html) {
  ...
  if (!title) {
    ...
    if (titleTag) title = cleanText(titleTag[1].split('|')[0].split('–')[0]);
  }
  ...
  // Skip if it matches a junk title pattern
  if (title && JUNK_TITLES.some(rx => rx.test(title))) return null;
...
function extractDate(html) {
  // Layer 1: JSON-LD structured data — most reliable (The Events Calendar, Schema.org)
  // Matches "startDate":"2026-06-12" or "startDate": "2026-06-12T18:00:00"
  const jsonld = html.match(/"startDate"\s*:\s*"(202\d-(?:0[1-9]|1[0-2])-(?:0[1-9]|[12]\d|3[01]))/i);
  if (jsonld) return jsonld[1].slice(0, 10);

  // Layer 2: <time datetime="YYYY-MM-DD"> elements (exclude those in nav/footer/aside)
  // We look specifically for structured time tags near event-detail wording
  const timeTag = html.match(/<time[^>]+datetime="(202\d-(?:0[1-9]|1[0-2])-(?:0[1-9]|[12]\d|3[01]))[^"]*"/i);
  if (timeTag) return timeTag[1].slice(0, 10);

  // Layer 3: Text regex — but ONLY on the main-content portion of the page.
  // Truncate HTML at the first "related events / upcoming events" section to
  // avoid grabbing dates from sidebar cards (the root cause of the bug).
  const RELATED_MARKERS = [
    /related[\s_-]*events?/i,
    /upcoming[\s_-]*events?/i,
    /more[\s_-]*events?/i,
    /you[\s_-]*may[\s_-]*also[\s_-]*like/i,
    /other[\s_-]*events?/i,
    /event[\s_-]*spotlight/i,
  ];

  let searchHtml = html;
  for (const marker of RELATED_MARKERS) {
    const markerIdx = html.search(marker);
    if (markerIdx > 500) { // must be well into the page, not in nav
      searchHtml = html.slice(0, markerIdx);
      break;
    }
  }

  // ISO date in content or JSON-LD within truncated section
  const iso = searchHtml.match(/\b(202\d-(?:0[1-9]|1[0-2])-(?:0[1-9]|[12]\d|3[01]))\b/);
  if (iso) return iso[1];

  // Human format: "June 12, 2026" or "Mar 14, 2026"
  const months = 'Jan(?:uary)?|Feb(?:ruary)?|Mar(?:ch)?|Apr(?:il)?|May|Jun(?:e)?|Jul(?:y)?|Aug(?:ust)?|Sep(?:tember)?|Oct(?:ober)?|Nov(?:ember)?|Dec(?:ember)?';
  const human  = searchHtml.match(new RegExp(`(${months})\\s+(\\d{1,2}),?\\s+(202\\d)`, 'i'));
  if (human) {
...
const CATEGORY_MAP = [
  ...
  [/\/neighborhood/i,            'Discovery',        'Neighborhoods'],
